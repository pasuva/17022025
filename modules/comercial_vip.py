...
def viabilidades_section():
    ...
    if st.session_state.viabilidad_marker:
        ...
        # Solo mostrar el formulario de precontrato si hay apartment_id
        if apartment_id and apartment_id.strip() not in ["", "N/D"]:
            form_key = f"{lat}_{lon}"  # Para diferenciar formularios
            with st.expander("📑 Formulario de Precontrato", expanded=False):
                st.markdown(f"Formulario de precontrato para Apartment ID: {apartment_id}")
                ...
                with st.form(key="form_precontrato"):
                    ...
                    st.text_input("🏢 Apartment ID", value=str(apartment_id), disabled=True,
                                  key=f"apartment_id_{form_key}")
                    ...
                    if not precio:
                        ...
                        try:
                            ...
                            st.warning(
                                "⚠️ El precio debe ser un número válido, con coma si tiene decimales (Ej: 1200,50).")

                    # Validación y formateo
                    if precio:
                        # Reemplaza punto por coma si el usuario escribe punto
                        precio_formateado = precio.replace(".", ",")
                        try:
                            # Intentamos convertir a float usando coma como decimal
                            precio_float = float(precio_formateado.replace(",", "."))
                            # Lo mostramos con coma siempre
                            precio_formateado = f"{precio_float:,.2f}".replace(",", "X").replace(".", ",").replace("X",
                                                                                                                   ".")
                        except ValueError:
                            st.warning("Introduce un precio válido, usando coma para los decimales.")
                    ...
                    st.subheader("📡 Líneas Asociadas al Precontrato")
                        ...
                        with st.expander(f"Línea móvil adicional #{i}", expanded=False):
                            ...
                            with col1:
                                tipo = st.selectbox("Tipo", ["nuevo", "portabilidad"],
                                                    key=f"adicional_tipo_{i}_{form_key}")
                                numero = st.text_input("Número a portar / nuevo",
                                                       key=f"adicional_numero_{i}_{form_key}")
                            with col2:
                    ...
                    if submit_precontrato:
                        try:
                            ...
                            if not token_valido:
                                ...
                                link_cliente = f"{base_url}/formulario_cliente?id={precontrato_pk}&token={urllib.parse.quote(token)}"

                                st.toast("✅ Precontrato guardado correctamente.")
                                st.markdown(
                                    f"📎 **Enlace para el cliente (válido 24 h):**\n\n[{link_cliente}]({link_cliente})")
                                st.info(
            ...
            st.info("⚠️ Debes asignar un Apartment ID a la viabilidad para poder crear un precontrato.")
